...
- T67 後の pin chore: mise/npm の 7 日クールダウン(サプライチェーン保護、
  upgrade-tools.sh --before 7d)により 0.84.2(8/14、2 日齢)は方針上インストール不可。
  ポリシーバイパスは不採用とし、**ピンを 0.84.1(8/7、9 日齢)へ変更**。T65b で
  検証定数と全ソース引用アンカー(settings-manager/loader/types/security.md)を
  v0.84.1 タグに対して再確認・更新する。0.84.2 が 7 日齢に達した後の昇格は staleness/
  doctor 管理下の通常手順。
