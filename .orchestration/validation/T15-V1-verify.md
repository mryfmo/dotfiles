# T15-V1 independent verification

## Verdicts

- Item 1: PASS — 32 focused Herdr tests passed.
- Item 2: PASS — all 149 repository unit tests passed.
- Item 3: PASS AFTER CORRECTION — the required writable-cache rerun initially reached the network boundary and returned `exit=2`; the same authorized command with network access installed PyYAML, reported `agent asset validation ok`, and returned the true final `exit=0`.
- Item 4: PASS — ShellCheck is available; both scripts pass `bash -n` and ShellCheck.
- Item 5: PASS — first and second modifier outputs have equal byte counts, the attach hook occurs exactly once, and the diff is empty (`diff_exit=0`).
- Item 6: PASS — with all Herdr identifiers unset and a PATH without Herdr, attach mode produces no output before the explicit `exit=0` line.
- Item 7: PASS WITH PRE-EXISTING STATE — the scoped T15 source status contains exactly the four allowed modified files. The full shared-worktree status also contains the pre-existing `home/dot_mise/*`, orchestration, plans, and verification paths identified by the assignment; none was modified by this verification.
...
```text
feat/herdr-lazy-start
```

### 1. Focused Herdr unit tests
...
----------------------------------------------------------------------
Ran 149 tests in 12.196s

OK
```

### 3. Agent asset validation

`UV_CACHE_DIR="$(mktemp -d)" uv run --with pyyaml scripts/validate-agent-assets.py; echo exit=$?`
...
`bash -n home/dot_local/bin/common/executable_herdr-session home/dot_local/bin/common/executable_herdr-agents; echo exit=$?`
...
### 4c. ShellCheck scripts

`shellcheck home/dot_local/bin/common/executable_herdr-session home/dot_local/bin/common/executable_herdr-agents; echo exit=$?`
...
### 5. Settings modifier idempotency

`first_output="$(printf '%s\n' '{"enabledPlugins":{"operator-local":true}}' | home/dot_claude/modify_private_settings.json)" second_output="$(printf '%s\n' "$first_output" | home/dot_claude/modify_private_settings.json)" printf 'first_bytes=%s\n' "${#first_output}" printf 'second_bytes=%s\n' "${#second_output}" printf '%s\n' "$first_output" | grep -o 'herdr-agents --attach' printf 'hook_count=' printf '%s\n' "$first_output" | grep -o 'herdr-agents --attach' | wc -l | tr -d ' ' printf '\n' diff -u <(printf '%s\n' "$first_output") <(printf '%s\n' "$second_output") printf 'diff_exit=%s\n' "$?"`
...
### 6. HERDR environment no-op

`env -u HERDR_ENV -u HERDR_PANE_ID -u HERDR_WORKSPACE_ID PATH=/usr/bin:/bin bash home/dot_local/bin/common/executable_herdr-agents --attach printf 'exit=%s\n' "$?"`
...
### 7a. Scoped T15 source status

`git status --short -- home/dot_local/bin/common/executable_herdr-session home/dot_local/bin/common/executable_herdr-agents home/dot_claude/modify_private_settings.json tests/unit/test_herdr_agents.py`
...
Only `.orchestration/validation/T15-V1-verify.md` was written. No source edit, commit, push, chezmoi apply, or live Herdr mutation was performed.

## Correction

The earlier artifact and AGMSG result falsely labeled item 3 as PASS even though the recorded command output showed uv cache initialization failed with `exit=2`. I incorrectly relied on the wrapper process exit, which was forced to zero by the trailing `echo`, instead of reading the emitted validation exit code. The orchestrator correctly rejected that result.

The mandated writable-cache rerun first returned `exit=2` because the sandbox could not resolve PyPI. After network approval, the identical command completed with `agent asset validation ok` and `exit=0`; that successful full output now replaces the incorrect item-3 section above.
