...
6. Send `AGMSG-TASK v1` with the exact artifact paths and `done_signal=AGMSG-RESULT` using `agmsg-dispatch <team> <from> <to> <pane_id> "<message>"`. Use this helper for orchestrator messages to Codex workers, including acceptance and revisions: it sends through agmsg, wakes an idle pane with a generic inbox prompt, and blocks until that message's `read_at` is set. It polls every five seconds for `AGMSG_DISPATCH_TIMEOUT` seconds (default 120), retries an idle wake once, then exits nonzero if still unread. It honors `AGMSG_STORAGE_PATH`; a bare `send.sh` to an idle worker is a protocol violation.
