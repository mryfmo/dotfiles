...
SKILL_HOOK_PATH_PREFIX="$SKILL_DIR/scripts/"
...
# entry is "agmsg-owned" when one of its inner hooks references a managed
# script path under our skill directory. Result is written back to <path>
# atomically.
#
    ...
    SELECT CASE
      ...
      WHEN (SELECT count(*) FROM json_each(json_extract(src.j, '\$.hooks.$event')) AS s
            WHERE NOT EXISTS (
              ...
              WHERE instr(json_extract(h.value, '\$.command'), '$SKILL_HOOK_PATH_PREFIX') > 0
            )) = 0 THEN
...
restart) do_restart "$@" ;;
*)
  echo "Unknown action: $ACTION (use set|status|stop|restart)" >&2
  exit 1
  ;;
esac
