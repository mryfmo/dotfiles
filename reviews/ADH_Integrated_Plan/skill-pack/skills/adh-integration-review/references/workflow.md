...
対象role：A1。既知baseと候補を直列統合し、結果が変わったら新snapshotで検証する。個別PASSの足し算で全体合格にしない。remote push/merge/publishの権限は別に確認する。
...
出力：統合順序、新snapshot、統合検証、失効/修正判断。失敗・未実施・外部待ちは原本参照付きで残す。必要な検査はstage別inventoryから選び、独立/統合/最終ゲートは省略しない。
