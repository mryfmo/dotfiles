# schema migrationの実行契約

対象role：A2。承認済みschema変更と既存データ契約を確認し、migration・失敗時・復旧を検証する。破壊的な本番適用は委任を別に確認する。単にDBという語が出ただけでは適用しない。
...
出力：migration差分、旧データ保持、適用/復旧試験結果。失敗・未実施・外部待ちは原本参照付きで残す。必要な検査はstage別inventoryから選び、独立/統合/最終ゲートは省略しない。
