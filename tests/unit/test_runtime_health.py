...
class RuntimeHealthTest(unittest.TestCase):
    ...
    def test_make_doctor_propagates_runtime_drift_after_tool_checks(self) -> None:
        ...
        self.executable(repo / "scripts/check-agent-runtime.py", "printf 'runtime drift\\n' >&2\nexit 7\n")
    ...
    def test_make_doctor_does_not_skip_runtime_check_when_deployed_root_is_missing(self) -> None:
        ...
        shutil.copy(ROOT / "Makefile", repo / "Makefile")
        shutil.copy(ROOT / "scripts/check-tools.sh", repo / "scripts/check-tools.sh")
        self.executable(repo / "scripts/check-agent-runtime.py", "printf 'missing runtime root\\n' >&2\nexit 7\n")
        env = self.doctor_environment()
        env["HOME"] = str(home)
